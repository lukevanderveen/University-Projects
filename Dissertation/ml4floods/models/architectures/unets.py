...
class UNet(nn.Module):
    ...
    # method for penultimate latent feature extraction
    def get_penultimate_layer(self, x):
        # Perform the forward pass until dconv_up1
        conv1 = self.dconv_down1(x)
        x = self.maxpool(conv1)
        ...
        penultimate = self.dconv_up1(x)  # Return this

        # Debug: Ensure the penultimate layer has the expected shape
        print(f"Penultimate layer shape: {penultimate.shape}")
