...
class WorldFloodsModel(pl.LightningModule):
    ...
    def validation_step(self, batch: Dict, batch_idx):
        ...
        for k in recall.keys():
            self.log(f"val_recall {k}", recall[k], sync_dist=True)

        # Log IoU per class
        iou_dict = metrics.calculate_iou(cm_batch, self.label_names)
        for k in iou_dict.keys():
            self.log(f"val_iou {k}", iou_dict[k], sync_dist=True)
...
class ML4FloodsModel(pl.LightningModule):
    ...
    def validation_step(self, batch: Dict, batch_idx):
        ...
        for i in range(len(self.label_names)):
            ...
            self.log(f"val_Acc_{problem_name}", metrics.binary_accuracy(cm_agg), sync_dist=True)
            self.log(f"val_Precision_{problem_name}", metrics.binary_precision(cm_agg), sync_dist=True)
            self.log(f"val_Recall_{problem_name}", metrics.binary_recall(cm_agg), sync_dist=True)

            bce = losses.binary_cross_entropy_loss_mask_invalid(logits[:, i], y[:, i], pos_weight=None)
            self.log(f"val_bce_{problem_name}", bce, sync_dist=True)

            # Log IoU per class
            iou_dict = metrics.calculate_iou(cm_batch, self.label_names[i, 1:])
            for k in iou_dict.keys():
                self.log(f"val_iou_{problem_name} {k}", iou_dict[k], sync_dist=True)
