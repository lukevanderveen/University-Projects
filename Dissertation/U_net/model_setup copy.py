import torch
import itertools
from worldfloods_model import WorldFloodsModel, ML4FloodsModel, load_weights
...
def get_pred_function(model: torch.nn.Module, device:torch.device, module_shape: int=1, max_tile_size: int=128,
    ...
    if module_shape > 1:
        ...
        # def pred_fun(ti, model = model, activation_fun = activation_fun , device = device):
        #     ti = ti.to(device)              # Move the tensor to the specified device
        #     output = model(ti)              # Pass the tensor through the model
        #     activated_output = activation_fun(output)  # Apply the activation function
...
def predbytiles(pred_function: Callable[[torch.Tensor], torch.Tensor], input_batch: torch.Tensor,
                tile_size=1280, pad_size=0, device=torch.device("cpu"),
    ...
    ori_input_batch_shape = input_batch.shape

    #padding the batch so it could be divided by the tile size

    SIZE_X = (input_batch.shape[2]//tile_size+1)*tile_size #Nearest size divisible by our patch size(Larger than the original size)
    SIZE_Y = (input_batch.shape[3]//tile_size+1)*tile_size #Nearest size divisible by our patch size(Larger than the original size)

    img_padded = torch.zeros(input_batch.shape[0], input_batch.shape[1], SIZE_X, SIZE_Y, device=device)

    img_padded[:,:, :input_batch.shape[2], :input_batch.shape[3]] = input_batch

    input_batch = img_padded
    ...
    pred_continuous_tf = pred_continuous_tf[:, :, :ori_input_batch_shape[2], :ori_input_batch_shape[3]]
